...
print(speclines)
print(speclines_name)

# -------------------------------


def get_spectrallines(flux, wavelength, z, sigma=4, delta=10):
    ...
    s = 1 # !! change into loop over speclines
    ...
    interval_flux = smoothflux[(speclines[s] - delta < wavelength) & (wavelength < speclines[s] + delta)]
    interval_wavelength = wavelength[(speclines[s] - delta < wavelength) & (wavelength < speclines[s] + delta)]
    interval_gradient = gradient[(speclines[s] - delta < wavelength) & (wavelength < speclines[s] + delta)]

    # Check if the derivative changes sign in this interval
    for i in range(len(interval_flux)-1):
        if (interval_gradient[i] < 0) != (interval_gradient[i+1] < 0):

            # Check if it's a maximum or a minimum
            if interval_flux[i-1] > interval_flux[i] and interval_flux[i+1] > interval_flux[i]:
                # minimum
                print("Minimum")
            elif interval_flux[i-1] < interval_flux[i] and interval_flux[i+1] < interval_flux[i]:
                # maximum
                print("Maximum")

            wavelength_mid = interval_wavelength[i]
            break

        else: # If no critical point is found
            wavelength_mid = speclines[s]
